...
def generate_random_tuples(num_tuples=10, lower_bound=-0.8, upper_bound=0.8):
    return [(random.uniform(lower_bound, upper_bound), random.uniform(lower_bound, upper_bound)) for _ in range(num_tuples)]

source_list = generate_random_tuples()
print(source_list)
path = "../pre_trained_models/FLIPY_NEW_PINN_conditioned_300000_100_1.0_200_200_800_6_64_tanh_0.07.pth"
index = 0
for source in source_list:
    ...
    class Pinns:
        ...
        def add_solid_points(self):
            ...
            n_different_sources = 1

            # Generate random source locations within the box of -0.5 to 0.5 for both x and y
            np.random.seed(40)  # To ensure repeatability
            source_x = np.random.uniform(-0.0001,0.0001, n_different_sources)
            source_y = np.random.uniform(-0.0001,0.0001, n_different_sources)
    ...
    tag = "conditioned_{}".format(index)+path.replace(".pth","")
    ...
    my_network = NeuralNet(input_dimension=5, output_dimension=2,n_hidden_layers=6,neurons=n_neurons,regularization_param=0.,regularization_exp=2.,retrain_seed=42)
