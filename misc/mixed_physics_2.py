...
class Pinns:
    def __init__(self, n_collocation_points,interval):
        ...
        self.training_set_s, self.training_set_f, self.training_set_b = self.assemble_datasets(interval)
    ...
    def redraw(self,interval):
        ...
        return inp_train_s,inp_train_f,inp_train_b

    def convert(self, tens):
        assert (tens.shape[1] == self.domain_extrema.shape[0])
        return tens * (self.domain_extrema[:, 1] - self.domain_extrema[:, 0]) + self.domain_extrema[:, 0]

    def add_solid_points(self,interval):
        input_s = self.convert(self.soboleng.draw(self.n_collocation_points))
        input_s[:, 0] = input_s[:, 0] * (interval[1] - interval[0]) / 2 + (interval[1] + interval[0]) / 2
    ...
    def add_fluid_points(self,interval):
        ...
        #print("input f = ", input_f, interval)
        # Ensure that the minimum y value does not reach 0
        input_f[input_f[:, 2] == 0, 2] += 1e-8

        return input_f
    def add_boundary_points(self,interval):
        # draw points from range -1 to 1

        input_b = self.convert(self.soboleng.draw(self.n_collocation_points))
        input_b[:, 0] = input_b[:, 0] * (interval[1] - interval[0]) / 2 + (interval[1] + interval[0]) / 2

        input_b[:, 2] = torch.full(input_b[:, 2].shape, 0.0)
        #print("input b = ", input_b, interval)

        return input_b

    # Function returning the training sets as dataloader
    def assemble_datasets(self,interval):
        input_s = self.add_solid_points(interval)
        input_f = self.add_fluid_points(interval)
        input_b = self.add_boundary_points(interval)
        training_set_s = DataLoader(torch.utils.data.TensorDataset(input_s),batch_size=self.n_collocation_points, shuffle=False, num_workers=4)
        ...
        training_set_b = DataLoader(torch.utils.data.TensorDataset(input_b),batch_size=self.n_collocation_points, shuffle=False, num_workers=4)
    ...
    def compute_loss(self,inp_train_s, inp_train_f, inp_train_b,verbose):
    ...
    def fit(self, num_epochs, optimizer, verbose=False):
        ...
        inp_train_s, inp_train_f, inp_train_b = self.redraw([-1,-1])

        #print("type = ",type(inp_train_s),inp_train_s)
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 5 == 0:
                interval = [-1,2.0*(float(epoch/num_epochs)) - 1.0]
                #inp_train_s[:, 0] = inp_train_s[:, 0] * (interval[1] - interval[0]) / 2 + (interval[1] + interval[0]) / 2
                #inp_train_f[:, 0] = inp_train_f[:, 0] * (interval[1] - interval[0]) / 2 + (interval[1] + interval[0]) / 2
                #inp_train_b[:, 0] = inp_train_b[:, 0] * (interval[1] - interval[0]) / 2 + (interval[1] + interval[0]) / 2
                inp_train_s, inp_train_f, inp_train_b = self.redraw(interval)
            if epoch % 50 == 0:
                time_list = [-1.0,-0.9,-0.8,-0.7,-0.5,0.0]
                res_list_ux = []
                res_list_uy = []
                numpoints_sqrt = 512
            ...
            def closure():
                optimizer.zero_grad()
                loss = self.compute_loss(inp_train_s, inp_train_f, inp_train_b,verbose=verbose)
                loss.backward()
...
wandb.init(project='Semester Thesis',name = ' ALL 3 Losses  Time marching my ansatz4 more iters less epochs ')
pinn = Pinns(n_points_per_training_set,interval=[-1,-1])
n_epochs = 1000
