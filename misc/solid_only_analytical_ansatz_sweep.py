...
class Pinns:
    ...
    def fit(self, num_epochs, optimizer, verbose=False):
        ...
        for epoch in range(num_epochs):
            ...
            num_samples = min(len(inp_train_s), (epoch + 1) * (int(self.n_collocation_points/num_epochs)))
...
wandb.init(project='Semester Thesis',name = 'test')
pinn = Pinns(n_points_per_training_set)
n_epochs = wandb.config.epochs
optimizer_LBFGS = optim.LBFGS(pinn.approximate_solution.parameters(),
                              lr=float(wandb.config.learning_rate),
                              max_iter=wandb.config.max_iter,#200
                              max_eval=wandb.config.max_eval, #200
                              history_size=wandb.config.history_size, #2000
                              line_search_fn="strong_wolfe",
                              tolerance_grad=wandb.config.tolerance_grad, #1e-8
                              tolerance_change=1.0 * np.finfo(float).eps)
hist = pinn.fit(num_epochs=n_epochs,
                optimizer=optimizer_LBFGS,
                verbose=True)
torch.save(pinn.approximate_solution.state_dict(),
           "../pre_trained_models/analytical_ansatz_m.pth")
