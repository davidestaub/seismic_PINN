...
class Pinns:
    ...
    def pinn_model_eval(self, input_tensor):
        ...
        U[:, 0] = U_perturbation[:, 0] * torch.tanh(torch.tanh((t + 1.0) * a)) + u0x * torch.sigmoid(
            15.0 * (b - ((t + 2.75) * 1.0)))
        U[:, 1] = U_perturbation[:, 1] * torch.tanh(torch.tanh((t + 1.0) * a)) + u0y * torch.sigmoid(
            15.0 * (b - ((t + 2.75) * 1.0)))
        return U
